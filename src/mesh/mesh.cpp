...
Mesh::Mesh(GLfloat* vertices, GLsizeiptr verticesSize, GLuint* indices, GLsizeiptr indicesSize, GLsizei stride) {
...
void Mesh::draw(Window& window, Shader& shader, glm::vec3 position, glm::vec2 scale, float rotation, glm::vec2 texRepeat) {
  ...
  shader.setMat4("projection", glm::ortho(0.f, static_cast<float>(window.getWidth()), static_cast<float>(window.getHeight()), 0.f));
  shader.setVec2("texRepeat", texRepeat);

  // Set color if not textured
  if (!textured) shader.setVec3("aColor", color);
...
void Mesh::draw(Window& window, Camera& camera, Shader& shader, glm::vec3 position, glm::vec3 scale, glm::vec3 rotation, glm::vec2 texRepeat) {
  // Create the model matrix
  glm::mat4 model(1.f);
...
void Mesh::updateMesh(GLfloat* vertices, GLsizeiptr verticesSize, GLuint* indices, GLsizeiptr indicesSize) {
  if (indices == 0 && indicesSize == 0) useEBO = false; else useEBO = true;
