...
int main(int argc, char *argv[]) {
  ...
  Shader framebufferShader("shaders/screen_vert.glsl",
  ...
  while (!window.shouldClose()) {
    ...
    if (window.isKeyPressed(GLFW_KEY_F)) {
      RayCast ray(camera.position, glm::normalize(camera.direction), 100.f,
                  objects);

      if (ray.hasHit()) {
        HitTarget target = ray.getHitTarget();
        std::cout << "Hit position: " << glm::to_string(target.position)
                  << std::endl;
        std::cout << "Surface normal: " << glm::to_string(target.normal)
                  << std::endl;
    ...
    for (auto &object : objects) {
      ...
      mesh.draw(window, camera, shader, object.position, object.scale,
                object.rotation, object.texRepeat);

      // Draw the collider
      if (settings.showColliders) {
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        shader.setBool("textured", false);
        mesh.draw(window, camera, shader, object.position, object.scale + 0.3f,
                  object.rotation, object.texRepeat);
    ...
    if (settings.skyboxEnabled)
      skybox.draw(window, camera, skyboxShader);
    ...
    if (!camera.isLocked) {
      ...
      for (unsigned int i = 0; i < objects.size(); i++) {
        ...
        if (objects[i].name.empty())
          std::sprintf(label, "ID) #%u", i);
        else
          std::sprintf(label, "%s", objects[i].name.c_str());
      ...
      if (ImGui::Button("Save")) {
        ...
        for (const auto &object : objects) {
          f << object.name << "," << object.position.x << ","
            << object.position.y << "," << object.position.z << ","
            << object.rotation.x << "," << object.rotation.y << ","
            << object.rotation.z << "," << object.scale.x << ","
            << object.scale.y << "," << object.scale.z << "," << object.color.x
            << "," << object.color.y << "," << object.color.z << ","
            << object.texRepeat.x << "," << object.texRepeat.y << ","
            << object.texture << "," << object.textured << "\n";
        }
      ...
      if (ImGui::Button("Load")) {
        ...
        while (getline(f, buffer)) {
          ...
          // Read position
          ss >> object.position.x;
          ss.ignore(1, ',');
          ss >> object.position.y;
          ss.ignore(1, ',');
          ss >> object.position.z;
          ss.ignore(1, ',');

          // Read rotation
          ss >> object.rotation.x;
          ss.ignore(1, ',');
          ss >> object.rotation.y;
          ss.ignore(1, ',');
          ss >> object.rotation.z;
          ss.ignore(1, ',');
          ...
          // Read texRepeat
          ss >> object.texRepeat.x;
          ss.ignore(1, ',');
          ss >> object.texRepeat.y;
          ss.ignore(1, ',');
      ...
      if (selectedObject < -1) {
        char buffer[256];
        std::strncpy(buffer, objects[selectedObject].name.c_str(),
                     sizeof(buffer));
        buffer[sizeof(buffer) - 1] = 0;
        if (ImGui::InputText("Name", buffer, sizeof(buffer)))
          objects[selectedObject].name = std::string(buffer);
        ImGui::TextColored(settings.ui.highlight, "Transform");
        ImGui::DragFloat3("Position",
                          glm::value_ptr(objects[selectedObject].position));
        ImGui::DragFloat3("Rotation",
                          glm::value_ptr(objects[selectedObject].rotation));
        ImGui::DragFloat3("Scale",
                          glm::value_ptr(objects[selectedObject].scale));
        if (objects[selectedObject].textured) {
          ...
          for (unsigned int i = 0; i < textures.size(); i++) {
            ...
            if (ImGui::ImageButton(
                    label.c_str(), (ImTextureID)(uintptr_t)textures[i].ID,
                    ImVec2(32.f, 32.f), ImVec2(0.f, 1.f), ImVec2(1.f, 0.f))) {
              objects[selectedObject].texture = textures[i].ID;
            }
            if (((i + 1) % 3 != 0 && i != textures.size() - 1))
              ImGui::SameLine();
          }
          ImGui::DragFloat2("Texture Repeat",
                            glm::value_ptr(objects[selectedObject].texRepeat));
        } else {
          ImGui::ColorEdit3("Color",
                            glm::value_ptr(objects[selectedObject].color));
        }
      ...
      static float vignetteIntensity =
          framebufferShader.getFloat("vignetteIntensity");
      if (ImGui::DragFloat("Vignette Intensity", &vignetteIntensity))
        framebufferShader.setFloat("vignetteIntensity", vignetteIntensity);
      static float vignetteInner = framebufferShader.getFloat("vignetteInner");
      if (ImGui::DragFloat("Vignette Inner", &vignetteInner))
        framebufferShader.setFloat("vignetteInner", vignetteInner);
      ImGui::End();
      window.uiPresent();
    }
